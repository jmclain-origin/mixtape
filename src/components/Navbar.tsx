...
const Navbar = () => {
  ...
  return (
    <nav
      className={`flex items-center justify-between px-3 py-2 sticky top-0 w-full transition-all ${
    ...
    >
      <span
        ...
        data-action="back"
      >
        {isPlaylist ? <FaBackward className="control-button" /> : <>&#10084;</>}
      </span>
      {!isHome && (
      ...
      <span
        ...
        data-action="forward"
      >
        {isPlaylist ? <FaForward className="control-button" /> : <>&#10084;</>}
      </span>
    </nav>
  );
};
